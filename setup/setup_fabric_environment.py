...
#Constants
certificateBucket = "fabric5510bucket-1"
...
if len(peers['Nodes']) == 0:
    ...
    print ("aws managedblockchain create-node --network-id " + blockchainNetwork['Id'] + " --member-id " + blockchainMember['Id'] + " --node-configuration 'InstanceType=bc.t3.medium,AvailabilityZone=ap-northeast-1'")
    sys.exit()
...
def store_public_certs():
    ...
    caPublicCerts3Key = blockchainNetwork['Id'] + "/" + blockchainMember['Id'] + "/admin-msp/cacerts/ca-" + blockchainMember['Id'] + "-" + blockchainNetwork['Id'] + "-ap-northeast-1-amazonaws-com.pem"
